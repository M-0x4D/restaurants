  ...
  <div class="mt-2 card">
    <div class="card-header d-flex justify-content-between">
       <div class="header-title">
          <h4 class="card-title">Update Week Hours</h4>
       </div>
    </div>
    <div class="card-body">
       <form action="{{ route('week_hours.update', $weekHour->id) }}" enctype="multipart/form-data" method="post" class="row g-3 needs-validation" novalidate>
          @method('PUT')
          @csrf
           ...
           <div class="row">
                   ...
                   <div class="form-group">
                       ...
                       <select class="form-select @error('restaurant_id') is-invalid @enderror" name="restaurant_id" id="exampleFormControlSelect1">
                           ...
                           @foreach ($restaurants as $restaurant)
                               <option value="{{ $restaurant->id }}" @if($weekHour->restaurant_id == $restaurant->id) selected @endif>{{ $restaurant->name }}</option>
                           @endforeach
                       </select>
                       <div class="invalid-feedback">
                           Please provide a valid Restaurants name.
                       </div>
                       @error('restaurant_id')
               ...
               <div class="col-md-6">
                   <div class="form-group">
                       <label class="form-label" for="exampleFormControlSelect1">Select Day</label>
                       <select class="form-select @error('day_id') is-invalid @enderror" name="day_id" id="exampleFormControlSelect1">
                           <option selected="" >Select the Day</option>
                           @foreach ($days as $day)
                               <option value="{{ $day->id }}" @if($weekHour->day_id == $day->id) selected @endif>{{ $day->name }}</option>
                           @endforeach
                       </select>
                       <div class="invalid-feedback">
                           Please provide a valid Restaurants name.
           ...
           <div class="row">
               <div class="col-md-6">
                   ...
                   <input type="time" class="form-control @error('from') is-invalid @enderror" name="from" value="{{$weekHour->from}}">
                   <div class="invalid-feedback">
               ...
               <div class="col-md-6">
                   ...
                   <input type="time" class="form-control @error('to') is-invalid @enderror" name="to" value="{{$weekHour->to}}">
                   <div class="invalid-feedback">
                       Please provide a valid Date.
                   </div>
                   @error('to')
