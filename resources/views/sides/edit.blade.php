  ...
  <div class="mt-2 card">
    <div class="card-header d-flex justify-content-between">
       <div class="header-title">
          <h4 class="card-title">Update Sides {{ $side->name }}</h4>
       </div>
    </div>
    <div class="card-body">
       <form action="{{ route('sides.update', $side->id) }}" enctype="multipart/form-data" method="post" class="row g-3 needs-validation" novalidate>
          @method('PUT')
          @csrf
          <div class="col-md-12">
             <label for="validationCustom01" class="form-label">name</label>
             <input type="text" name="name" class="form-control @error('name') is-invalid @enderror" value="{{ $side->name }}" id="validationCustom01" required>
          ...
          <div class="col-md-12">
            ...
            @enderror
            <img width="150" src="{{ $side->img_path }}" alt="" srcset="">
         </div>
         <div class="col-md-12">
            <label for="validationCustom03" class="form-label">Price</label>
            <input type="text" class="form-control @error('price') is-invalid @enderror" name="price" value="{{ $side->price }}">
            <div class="invalid-feedback">
